...
$('#changePwdBtn').addEventListener('click', () => {
  $('#pwdMsg').textContent = '';
  ['#pwd_current','#pwd_next','#pwd_next2'].forEach(i => $(i).value = '');
...
$('#pwdSave').addEventListener('click', async () => {
  const cur = $('#pwd_current').value, nx = $('#pwd_next').value, nx2 = $('#pwd_next2').value;
  const msg = $('#pwdMsg');
  if (!cur || !nx) { msg.textContent = 'Enter your current and new password.'; return; }
  if (nx.length < 6) { msg.textContent = 'New password must be at least 6 characters.'; return; }
  if (nx !== nx2) { msg.textContent = 'New passwords do not match.'; return; }
  try {
    const res = await api('POST','/api/me/password', { current: cur, next: nx });
    $('#pwdModal').style.display = 'none';
    toast('Password changed successfully');
    if (res.user && state.user) state.user.generatedPassword = res.user.generatedPassword;
  } catch (err) { msg.textContent = err.message; }
});
['#pwd_current','#pwd_next','#pwd_next2'].forEach(sel => $(sel).addEventListener('keydown', e => { if (e.key === 'Enter') $('#pwdSave').click(); }));
}
...
function renderSecretaryForm(content){
  ...
  <div class="card">
    ...
    <div class="muted" id="wdTotalLine">Total week day attendance: <strong>0</strong> persons</div>
    <table class="dyn-table"><thead><tr><th>Day</th><th>Activity</th><th>Children</th><th>Youth</th><th>Women</th><th>Men</th><th>Total</th><th></th></tr></thead>
...
function wireWeekday(){
  ...
  const updateTotal = () => {
    const t = freshForm.weekday.reduce((s,r)=>s+num(r.children)+num(r.youth)+num(r.women)+num(r.men),0);
    const line = $('#wdTotalLine'); if (line) line.innerHTML = 'Total week day attendance: <strong>' + t + '</strong> persons';
  };
  const render = () => {
    body.innerHTML = freshForm.weekday.map((d,i)=>`<tr data-i="${i}">
      ...
      <td class="dyn-total wd-tot">${num(d.children)+num(d.youth)+num(d.women)+num(d.men)}</td>
      <td><div class="row-actions"><button class="icon-btn del">🗑️</button></div></td></tr>`).join('');
    $$('#wdBody tr').forEach(tr => {
      const i = +tr.dataset.i; const r = freshForm.weekday[i];
      const bindNum = (cls,key) => { const el = $(cls,tr); if (el) el.addEventListener('input', () => { r[key]=el.value; const tot=$('.wd-tot',tr); if (tot) tot.textContent = num(r.children)+num(r.youth)+num(r.women)+num(r.men); updateTotal(); }); };
      bindNum('.wd-ch','children'); bindNum('.wd-yo','youth'); bindNum('.wd-wo','women'); bindNum('.wd-me','men');
      const day = $('.wd-day',tr); if (day) day.addEventListener('change', e => r.day = e.target.value);
      const act = $('.wd-activity',tr); if (act) act.addEventListener('change', e => { r.activity = e.target.value; if (r.activity !== 'Others') r.others=''; render(); updateTotal(); });
      const oth = $('.wd-others',tr); if (oth) oth.addEventListener('input', e => r.others = e.target.value);
      const del = $('.del',tr); if (del) del.addEventListener('click', () => { freshForm.weekday.splice(i,1); render(); updateTotal(); });
    });
  };
  render(); updateTotal();
  $('#wdAdd').addEventListener('click', () => { freshForm.weekday.push({day:'',activity:'',others:'',children:'',youth:'',women:'',men:''}); render(); updateTotal(); });
}
...
function openCalendar(input){
  ...
  function draw(){
    ...
    const prevY=document.createElement('button'); prevY.textContent='«';
    const prev=document.createElement('button'); prev.textContent='‹';
    const title=document.createElement('span'); title.textContent=MONTHS[vm]+' '+vy;
    const next=document.createElement('button'); next.textContent='›';
    const nextY=document.createElement('button'); nextY.textContent='»';
    prevY.title='Previous year'; prev.title='Previous month'; next.title='Next month'; nextY.title='Next year';
    prevY.onclick=()=>{ vy--; draw(); };
    prev.onclick=()=>{ vm--; if(vm<0){vm=11;vy--;} draw(); };
    next.onclick=()=>{ vm++; if(vm>11){vm=0;vy++;} draw(); };
    nextY.onclick=()=>{ vy++; draw(); };
    head.append(prevY,prev,title,next,nextY); pop.appendChild(head);
